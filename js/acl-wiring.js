 ...
 *
 * Verwendung im Plugin — eine Datei neben dem Admin-Quellcode:
 *
 *     const checkAclWiring = require('../../../../../vendor/depa/shopware-extension-checks/js/acl-wiring');
 *
 *     test('jede Admin-Oberfläche nennt ein Recht', () => {
 ...
 *
 * Gelesen wird das **echte registrierte Objekt**, nicht der Dateitext: Der Stub fängt
 * `Module.register` ab und lässt alles andere ins Leere laufen. Eine Vorgänger-Fassung las den
 * Quelltext mit gezählten Klammern und brauchte dafür 469 Zeilen — dieselbe Auskunft.
 */
 ...
 *
 * @param {() => void} loadTheAdmin Lädt den Admin — im Plugin `() => require('./main.js')`.
 * @returns {{ modules: string[], open: string[] }}
 */
module.exports = function checkAclWiring(loadTheAdmin) {
    const modules = [];
...
if (require.main === module) {
    ...
    assert.deepStrictEqual(module.exports(() => {}).modules, []);
